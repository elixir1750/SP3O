...
from slime.utils.sp3o import build_critic_value_loss_masks, in_dense_critic_only_warmup
...
def loss_function(
    ...
    if (
        ...
        and getattr(args, "critic_token_loss", False)
        and not in_dense_critic_only_warmup(args, batch)
    ):
        loss_masks = build_critic_value_loss_masks(args, batch["loss_masks"])

    num_tokens = sum([torch.clamp_min(loss_mask.sum(), 1) for loss_mask in loss_masks])
